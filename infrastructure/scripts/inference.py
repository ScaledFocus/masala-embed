...
class UnifiedModel:
    ...
    def _load_r2_model(self):
        ...
        if self.model_config["r2_framework"] == "llama_cpp":
            ...
            raise ValueError(
                f"Unsupported R2 framework: {self.model_config['r2_framework']}"
            )
    ...
    def _load_llama_cpp_from_r2(self):
        ...
        if file_size < 1024 * 1024:  # Less than 1MB
            raise ValueError(
                f"Downloaded model seems corrupted (size: {file_size} bytes)"
            )
    ...
    def _load_hf_model(self):
        ...
        if self.model_config["hf_framework"] == "transformers":
            ...
            raise ValueError(
                f"Unsupported HF framework: {self.model_config['hf_framework']}"
            )
    ...
    def _embed_text(self, text: str) -> dict[str, Any]:
        ...
        if self.model_config["model_source"] == "r2":
            ...
            if self.processor is not None and (
                hasattr(self.processor, "tokenize")
                or hasattr(self.processor, "__call__")
            ):
