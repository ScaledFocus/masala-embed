...
def create_themed_prompt_variation(
    ...
    for pattern_name, weight in pattern_weights.items():
        ...
        themed_prompt = themed_prompt.replace(
            f"**{pattern_name} (", f"**{pattern_name} ({weight}%):**\nEMPHASIZED: "
        )
        themed_prompt = themed_prompt.replace(
            f"**{pattern_name} ({weight}%):**\nEMPHASIZED: ", old_pattern
        )

    return themed_prompt


def generate_themed_intents(
    theme_name: str,
    ...
    themed_prompt = themed_prompt.replace(
        "Generate 100 diverse", f"Generate {num_intents} diverse"
    )
    themed_prompt = themed_prompt.replace(
...
def create_shuffled_intent_sets(
    all_themed_intents: dict[str, list[str]], num_sets: int = 10
) -> list[dict[str, Any]]:
    ...
    for i in range(num_sets):
        ...
        end_idx = (
            start_idx + intents_per_set_actual if i < num_sets - 1 else total_intents
        )
...
def save_intent_sets(intent_sets: list[dict[str, Any]], output_dir: str) -> list[str]:
    ...
    summary = {
        "generation_info": {
            ...
            "creation_date": datetime.now().isoformat(),
            "intents_per_set": intent_sets[0]["metadata"]["intents_per_set"]
            if intent_sets
            else 0,
        },
...
def main():
    ...
    try:
        ...
        intent_sets = create_shuffled_intent_sets(all_themed_intents, args.num_sets)
        ...
        # Print summary
        total_themed_intents = sum(
            len(intents) for intents in all_themed_intents.values()
        )
