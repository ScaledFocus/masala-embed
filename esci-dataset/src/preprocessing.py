...
def combine_text_columns(row: pd.Series, columns: list[str]) -> str:
    ...
    for col in columns[1:]:
        ...
        # Handle list-like strings (e.g., ingredients stored as string
...
def deduplicate_records(
    ...
    logger.debug(
        f"Removed: {len(dedup_result.filtered)} "
        f"({len(dedup_result.filtered) / len(records) * 100:.1f}%)"
    )
...
def remove_null_rows(df: pd.DataFrame, columns: list[str]) -> pd.DataFrame:
    ...
    final_count = len(df)
    logger.info(
        f"Removed {initial_count - final_count} rows with null/empty values "
...
def _clean_consumable_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "consumable_nutritional_profile" in df.columns:
        df["consumable_nutritional_profile"] = df[
            "consumable_nutritional_profile"
        ].apply(_parse_json_field)
...
def deduplicate_dataset(
    ...
    if filtered_records:
        filtered_indices = [
            record.record["original_index"] for record in filtered_records
        ]
...
def hybrid_deduplicate_dataset(
    ...
    Preserves dish name diversity while removing semantic duplicates within each
    dish type.
    ...
    for dish_name in tqdm(dish_names, desc="Deduplicating by dish_name"):
        ...
        original_to_relative = {
            orig_idx: rel_idx for rel_idx, orig_idx in enumerate(dish_group.index)
        }
        kept_relative_indices = [
            original_to_relative[record["original_index"]]
        ...
        logger.debug(
            f"{dish_name}: {len(dish_group)} -> {len(dish_deduplicated)} records"
        )
...
def _process_dish_group(args):
    ...
    kept_relative_indices = [
        original_to_relative[record["original_index"]] for record in selected_records
    ]
    dish_deduplicated = dish_group_reset.iloc[kept_relative_indices].copy()
...
def hybrid_deduplicate_dataset_parallel(
    ...
    dish_names = df["dish_name"].unique()
    logger.info(
        f"Processing {len(dish_names)} unique dish names with parallel processing"
    )
    ...
    for dish_name in dish_names:
        ...
        process_args.append(
            (dish_name, dish_group_data, combined_text_column, threshold)
        )
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_dish = {
            executor.submit(_process_dish_group, args): args[0] for args in process_args
        }

        # Collect results with progress bar
        for future in tqdm(
            as_completed(future_to_dish),
            total=len(future_to_dish),
            desc="Deduplicating by dish_name",
        ):
    ...
    df_final.to_csv(output_path, index=False)
    logger.info(
        f"Parallel hybrid deduplication complete: {len(df_final)} records "
