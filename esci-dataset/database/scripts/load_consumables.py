...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("load_consumables.log"),
...
def main():
    ...
    parser.add_argument("--threshold", type=float, default=0.9, help="Deduplication threshold")
    ...
    try:
        ...
        if not args.dry_run:
            ...
            logger.info(f"Success rate: {insertion_stats['total_inserted']/len(df_prepared)*100:.1f}%")
        else:
            logger.info("\n" + "=" * 40)
...
def load_and_clean_data(input_file: str, data_folder: str = None) -> pd.DataFrame:
    ...
    if data_folder is None:
        ...
        if not data_folder:
            raise ValueError("data_folder not provided and not found in environment variables")
...
def insert_consumables_batch(df: pd.DataFrame, batch_size: int = 500) -> dict[str, int]:
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cursor:
            ...
            logger.info(f"Insertion completed: {stats['total_inserted']:,}/{stats['total_records']:,} records")
            logger.info(f"Database count: {initial_count:,} → {final_count:,} (+{final_count - initial_count:,})")
